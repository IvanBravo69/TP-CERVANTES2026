...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res?.success) { setError(res?.message || 'Error al iniciar sesi�n'); return }
    ...
    <div className="login-page">
        ...
        <div className="login-card">
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input className="form-control" style={{ paddingLeft:'2.25rem' }}
                  ...
                  placeholder="Ingres� tu usuario" autoComplete="username" required />
              </div>
            </div>
            ...
            <button type="submit" className="btn btn-primary w-100" style={{ height: 40 }} disabled={loading}>
              {loading ? <span className="spinner" style={{ width:16, height:16, borderWidth:2 }} /> : 'Iniciar Sesi�n'}
            </button>
          </form>
        </div>
      </div>
      ...
      <div className="login-right-panel">
        <div className="login-brand">
          ...
          <p>Plataforma integral de gesti�n inmobiliaria</p>
        </div>
        <div className="feature-list">
          {[
            { icon:'bi-people-fill',               title:'Gesti�n de Clientes',     desc:'Propietarios, inquilinos y compradores centralizados' },
            { icon:'bi-building-fill',             title:'Cat�logo de Propiedades',  desc:'Filtros por tipo, precio, ciudad y estado' },
            { icon:'bi-file-earmark-text-fill',    title:'Contratos y Finanzas',     desc:'Alquileres, ventas y registro de cobros' },
            { icon:'bi-shield-lock-fill',          title:'Roles y Permisos',         desc:'Control de acceso por �rea de trabajo' },
          ].map(f => (
