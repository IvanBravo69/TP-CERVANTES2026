...
export default function CobrosPage() {
  ...
  const load = useCallback(async (p = page) => {
    setLoading(true)
    try {
      const params = { page:p, limit:LIMIT, tipo:'Ingreso', ...Object.fromEntries(Object.entries(filters).filter(([,v]) => v !== '')) }
  ...
  async function handleSave() {
    setSaving(true)
    try {
      const payload = { ...modal.data, tipo:'Ingreso', monto: Number(modal.data.monto), contrato_id: modal.data.contrato_id || null }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res?.success) { toast.error(res?.message || 'Error'); return }
      toast.success('Cobro eliminado'); load(page)
    } catch(e) { toast.error(e?.message || 'Error') }
  }
  ...
  return (
    <>
      <div className="page-header" style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div><h1>Cobros</h1><p>Registro de ingresos</p></div>
        <button className="btn btn-primary" onClick={openModal}><i className="bi bi-plus-lg" /> Registrar cobro</button>
      </div>

      {Object.keys(totales).length > 0 && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(185px,1fr))', gap:'.875rem', marginBottom:'1rem' }}>
          {[
            { label:'Cobrado ARS', val: totales.ing_ars, color:'#15803d', bg:'#f0fdf4' },
            { label:'Cobrado USD', val: totales.ing_usd, color:'#1d4ed8', bg:'#eff6ff' },
          ].map(t => (
        ...
        <div className="table-wrapper">
          {loading ? <div style={{ textAlign:'center', padding:'3rem' }}><Spinner /></div> : (
            <table>
              ...
              <tbody>
                {rows.length === 0
                  ? <tr><td colSpan={6}><EmptyState icon="bi-arrow-down-circle" message="No hay cobros" /></td></tr>
                  : rows.map(r => (
                    <tr key={r.id}>
                      ...
                      <td style={{ fontWeight:600, color:'#15803d' }}>+ {r.moneda} {fmt(r.monto)}</td>
      ...
      <Modal open={modal.open} onClose={() => setModal(m => ({ ...m, open:false }))} title="Registrar cobro"
        footer={<>
      ...
      >
        ...
        <div className="form-group"><label className="form-label">Concepto *</label>
          <input className="form-control" value={modal.data.concepto} onChange={setF('concepto')} placeholder="Ej: Cuota Enero 2025, SeÃ±a" />
        </div>
        <div className="form-row">
      ...
      <ConfirmDialog open={confirm.open} onClose={() => setConfirm(c => ({ ...c, open:false }))} onConfirm={handleDelete}
        ...
        message={`Â¿Eliminar el cobro "${confirm.item?.concepto}"? Esta acciÃ³n no se puede deshacer.`}
      />
    </>
  )
}
